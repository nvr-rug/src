...
parser.add_argument('-repl', default = '', help='Replacing unknowns yes/no? Default is empty meaning no')
...
def process_dir(cp_direc):
	...
	replace_unk = '-replace_unk' if args.repl == 'true' else ''
	phrase_table = '-phrase_table /home/p266548/Documents/amr_Rik/Exp_OMT/Word_Experiments/phrase_table.txt' if 'Word_Experiments' in cp_direc else ''
	
	#print 'Replace unk: {0}'.format(replace_unk)
	print phrase_table
		...
		for root, dirs, files in os.walk(process_files):
			for f in files:
				...
				if f.endswith(args.test_ext):
					...
					test_call = 'th {0}translate.lua -src {1} -output {2} -model {3} -beam_size {4} -max_sent_length {5} {6} -n_best {7} {8} -log_file {9} {10} -fallback_to_cpu'.format(args.OMT_path, f_path, out_file, args.tf, args.beam_size, args.max_sent_length, replace_unk, args.n_best, gpu, log_output, phrase_table)
