...
def get_single_amr_line(input_f):
    for line in input_f:
        return line
...
def build_arg_parser():
    ...
    parser.add_argument('-b1', type=int,required = False, default = -1, help='Restart number (Default:4)')
    parser.add_argument('-b2', type=int, required = False, default = -1 , help='Restart number (Default:4)')
    parser.add_argument('-w1', type=int,required = False, default = -1, help='Restart number (Default:4)')
    parser.add_argument('-w2', type=int, required = False, default = -1, help='Restart number (Default:4)')
    ...
    parser.add_argument('--one_line', action='store_true', default= False, dest="one_line",
                      help="States if the input is already in one-line. Default: false")
    parser.add_argument('--both_one_line', action='store_true', default= False, dest="both_one_line",
                      help="States if the is both in one line. Default: false")                  
    return parser
...
def build_arg_parser2():
    ...
    parser.add_option('--pr', "--precision_recall", action='store_true', dest="pr",
                      help="Output precision and recall as well as the f-score. Default: false")
    parser.add_option('--one_line', action='store_true', dest="one_line",
                      help="States if the input is already in one-line. Default: false")
...
def check_sentence(sentence):   
    '''Do not count single punctuation characters as words for wordcount'''
...
def main(arguments):
    ...
    both_one_line = args.both_one_line
    #print args.f[0]
    #print args.f[1]
    ...
    if both_one_line:
        ...
        prod_amrs = [x.strip() for x in open(args.f[0],'r')]    
        gold_amrs = get_amr_line(args.f[1])
    else:
         prod_amrs = get_amr_line(args.f[0])
         gold_amrs = get_amr_line(args.f[1]) 
    if (args.b1 == -1 or args.b2 == -1) and (args.w1 == -1 or args.w2 == -1):
        raise ValueError("You have to specifiy a bound (b1 b2) or word range (w1 w2)")
    elif args.b1 == -1 and args.b2 == -1 and args.w1 == -1 and args.w2 == -1:
        raise ValueError("You cannot specifiy both b1 b2 AND w1 w2")
    else:
        if args.b1 >= args.b2 and (args.w1 == -1 or args.w2 == -1):
            raise ValueError ("bound1 should be smaller than bound2")          
        elif args.w1 >= args.w2 and (args.b1 == -1 or args.b2 == -1):
            raise ValueError ("w1 should be smaller than w2")
    idx = 0
    
    #print 'len gold', str(len(gold_amrs))
    #print 'len prod', str(len(prod_amrs))
    ...
    skipped = 0 # number of skipped sentences due to max length
    curr_idn = False
    curr_type = False
    
    full_sentences = []
    
    if args.w1 > -1 and args.w2 > -1:
        identifiers = ['consensus','wb','dfa','dfb','xinhua','bolt','proxy','cctv','guidelines','mt09sdl']
        type_ids = ['training','test','dev']
        ...
        if not curr_idn or not curr_type:
            raise ValueError ("Can't found valid ID in file")
        
        for root, dirs, files in os.walk('/home/p266548/Documents/amr_Rik/data_2017/data/amrs/split/{0}/'.format(curr_type)):
            for f in files:
                if curr_idn in f and f.endswith('.sent'):
                    f_path = os.path.join(root, f)
                    full_sentences = [x.strip() for x in open(f_path,'r')]                  
    
    for idx in range(len(gold_amrs)):
        #if one_line:
        #    cur_amr1 = get_single_amr_line(args.f[0])
        #else:
         #   cur_amr1 = get_amr_line(args.f[0])  
        #cur_amr2 = get_amr_line(args.f[1])
        
        # check if sentence falls within our range:
        if full_sentences:
            sentence = full_sentences[idx].split()
            sentence = check_sentence(sentence)
            if not(len(sentence) >= args.w1 and len(sentence) <= args.w2):
                skipped += 1
                continue    
        else:
            if not(len(gold_amrs[idx]) >= args.b1 and len(gold_amrs[idx]) <= args.b2):
                skipped += 1
                continue        
        
        cur_amr1 = prod_amrs[idx]
        cur_amr2 = gold_amrs[idx]
        #print 'amr-produced:', cur_amr1,'\n'
        #print 'amr-gold:', cur_amr2,'\n'
...
#            print "Smatch score: %.2f" % best_f_score
    ...
    if single_score:
        ...
        if pr_flag:
            print "Precision: %.2f" % precision
            print "Recall: %.2f" % recall
        print 'Skipped sentences: {0} out of {1}'.format(skipped, len(gold_amrs))
