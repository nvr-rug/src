...
def build_arg_parser():
    ...
    parser.add_argument('--sent', type=str, required = True, help='Sentence file of AMRs')
...
def filter_sent(full_sent):
    '''Filter sentence - do not count punctuation as words'''
    
    sent_spl = full_sent.strip().split()
    sent = []
...
def main(arguments):
    ...
    all_res = []
    sents = [x.strip() for x in open(args.sent,'r')]
    ...
    for idx in range(len(gold_amrs)):
        ...
        spl_sent = filter_sent(cur_sent)
        all_res.append([cur_amr1, cur_amr2, cur_sent, len(spl_sent), best_match_num, test_triple_num, gold_triple_num])
    ...
    if single_score:
        ...
        print "Document F-score: "+floatdisplay % best_f_score
    
    print 'Results per max sentence length'
    max_sen_length = max([x[3] for x in all_res])
    #max_sen_length = 50
    
    all_f = []
    all_max = range(1, max_sen_length+1)
    all_num_sents = []
    
    for max_len in range(1,max_sen_length + 1):
        keep_res = [x for x in all_res if x[3] <= max_len]
        total_best, total_test, total_gold = 0, 0, 0
        
        for item in keep_res:
            total_best += item[4]
            total_test += item[5]
            total_gold += item[6]
        
        (precision, recall, best_f_score) = compute_f(total_best, total_test, total_gold)
        all_num_sents.append(len(keep_res))
        all_f.append(best_f_score)
        print 'Max length {0}: {1}, {2} sents'.format(max_len, round(best_f_score, args.significant), len(keep_res))
    
    json_dump(all_f, '/home/p266548/Documents/amr_Rik/sem_eval_testdata/extra_analysis/json/f_scores_camr')
    json_dump(all_max, '/home/p266548/Documents/amr_Rik/sem_eval_testdata/extra_analysis/json/max_len_camr')             
    json_dump(all_num_sents, '/home/p266548/Documents/amr_Rik/sem_eval_testdata/extra_analysis/json/num_sents_camr')
