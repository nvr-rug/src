...
def get_accuracy(prod, gold):
	'''Function that returns accuracy of the symbolizer'''
	
	if not len(gold) == len(prod):
		raise ValueError("Produced and gold file are of different length")
	
	cor, inc = 0,0
	
	for p, g in zip(prod, gold):
		if p == g:
			cor += 1
		else:
			inc += 1
			if args.v:
				print '{0} instead of {1}'.format(p, g)
