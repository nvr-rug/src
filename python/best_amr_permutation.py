...
parser.add_argument("-cleanup", default = '', type=str, help="Delete all non-final files")
...
def smatch_test(old_amrs, new_amrs, rand_num):
	...
	for i,j in zip(sample_old, sample_new):
		...
		f_score_same_old = float(output.split()[-1])		# get F-score from comparing with itself
		
		output = subprocess.check_output(smatch_call(args.temp_new, args.temp_new), shell=True)
		f_score_same_new = float(output.split()[-1])		# get F-score from comparing with itself
		...
		if f_score < 0.8:
			...
			print f_score_same_old, f_score_same_new, f_score
		count += 1	
...
def check_validity(old_amrs_f, new_amrs_f, sent_amrs):
	...
	#smatch_test(old_amrs, new_amrs, 500)
